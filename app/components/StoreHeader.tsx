...
export default function StoreHeader({ name = "LojaKy", image, banner, rating = "4.9", address, deliveryTime = "30-45 min", restaurantId, ratingSum, ratingCount, instagram, whatsapp }: HeaderProps) {
    ...
    useEffect(() => {
        fetch('/api/config')
            // ... (existing fetch logic remains effectively unchanged as we are replacing the top part)
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <div className="relative mb-8">
                    ...
                    <button
                        onClick={() => {
                            if (navigator.share) {
                                navigator.share({
                                    title: name,
                                    text: `Confira a loja ${name} no LojAky!`,
                        ...
                        className="bg-white/90 backdrop-blur-sm text-gray-700 p-2 pr-4 rounded-full shadow-lg border border-gray-200 text-sm font-bold flex items-center gap-2 hover:bg-white transition-transform hover:scale-105 active:scale-95"
                    >
                        <div className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center text-lg border border-gray-200">
            ...
            {/* Content Section - Displayed below Top Bar - HIDDEN ON SIGNUP PAGE */}
            {!isSignupPage && (
